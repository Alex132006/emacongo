...
const userTypeOptions = document.querySelectorAll(
  "#registerFormContainer .user-type-option"
);
...
const loginTypeOptions = document.querySelectorAll(
  "#loginFormContainer .user-type-option"
);
...
document
  .querySelector('#registerFormContainer .user-type-option[data-type="buyer"]')
  .classList.add("selected");
document
  .querySelector('#loginFormContainer .user-type-option[data-type="buyer"]')
  .classList.add("selected");
...
function showDashboard(userData) {
  ...
  document.getElementById("userAccountType").textContent =
    userData.userType === "seller" ? "Vendeur" : "Acheteur";
  ...
  if (userData.userType === "seller") {
    ...
    if (userData.category) {
      const category = productCategories.find(
        (c) => c.id === userData.category
      );
      document.getElementById("userCategory").textContent = category
        ? category.name
        : userData.category;
    } else {
      document.getElementById("userCategory").textContent =
        "Aucune catégorie sélectionnée";
    }

    // Afficher les méthodes de paiement
    if (userData.paymentMethods && userData.paymentMethods.length > 0) {
      const paymentNames = userData.paymentMethods
        .map((paymentId) => {
          const payment = paymentMethods.find((p) => p.id === paymentId);
      ...
      document.getElementById("userPayments").textContent =
  ...
  if (validateLoginForm()) {
    ...
    if (users[email] && users[email].password === password) {
      ...
      if (userData.userType !== userType) {
        alert(
          `Erreur: Cet email est enregistré comme ${
            userData.userType === "seller" ? "vendeur" : "acheteur"
          }, mais vous essayez de vous connecter comme ${
            userType === "seller" ? "vendeur" : "acheteur"
          }`
        );
        return;
      }
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    if (validateRegisterForm()) {
      ...
      const message =
        `Inscription réussie! Bienvenue ${userData.name}!\n\n` +
...
function showDashboardActions() {
  const userType = document
    .getElementById("userAccountType")
  ...
  if (userType === "vendeur") {
    ...
    window.window.location.href = "acceuil.html";

    alert(
      "Actions vendeur disponibles:\n- Gérer les produits\n- Voir les commandes\n- Analyser les statistiques\n- Gérer la boutique" +
        categoryMessage +
        paymentsMessage
    );
  } else {
    alert(
      "Actions acheteur disponibles:\n- Parcourir les produits\n- Voir le panier\n- Consulter les commandes\n- Contacter les vendeurs\n\nLimitation: Vous ne pouvez que consulter"
    );
  }
}
